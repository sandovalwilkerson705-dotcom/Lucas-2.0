...
module.exports = async (msg, { conn }) => {
  try {
    ...
    if (mascota.cooldowns?.presumir && tiempoActual - mascota.cooldowns.presumir < cooldownTime) {
      ...
      return conn.sendMessage(
        msg.key.remoteJid,
        { text: `⏳ *Debes esperar ${tiempoRestante} minutos antes de volver a presumir a tu mascota.*` },
        { quoted: msg }
      );
    }

    // 🎖️ Generar recompensas aleatorias
    let diamantesGanados = Math.floor(Math.random() * 451) + 50;    // Entre 50 y 500
    let xpGanada        = Math.floor(Math.random() * 2301) + 500;  // Entre 500 y 2800
...
module.exports.command = ['presumir'];
