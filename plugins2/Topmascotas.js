...
module.exports = async (msg, { conn }) => {
  try {
    ...
    // 🐾 Construir array de ranking para mascotas
    let ranking = Object.entries(usuarios)
      .map(([id, user]) => {
        if (!user.mascotas || user.mascotas.length === 0) return null;
        const cantidad = user.mascotas.length;
        const totalNivel = user.mascotas.reduce((sum, m) => sum + (m.nivel || 1), 0);
        const listado = user.mascotas
          .map(m => `🎭 ${m.nombre} (Nivel ${m.nivel})`)
          .join("\n");
    ...
    let mensaje = "🏆 *Ranking de Jugadores con Más y Mejores Mascotas* 🏆\n━━━━━━━━━━━━━━━━━━━━\n";
    ranking.forEach((u, idx) => {
      mensaje += `🥇 *#${idx + 1} - @${u.id.split('@')[0]}*\n`;
      mensaje += `🐾 *Mascotas:* ${u.cantidad}\n`;
...
module.exports.command = ['topmascotas'];
